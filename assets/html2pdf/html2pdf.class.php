...
' . $content . '
            ...
            for ($x = 0; $x < count($corr[0]); $x++) {
                for ($y = 0; $y < count($corr); $y++) {
                    if (isset($corr[$y][$x]) && is_array($corr[$y][$x]) && $corr[$y][$x][2] > 1) {
                        ...
                        $s = 0;
                        for ($i = 0; $i < $corr[$y][$x][2]; $i++) $s += $sw[$x + $i];
        ...
        protected function _tag_open_TD($param, $other = 'td')
        {
            ...
            if ($this->_subPart) {
                ...
                for ($j = 0; $j < $rowspan; $j++) {
                    for ($i = 0; $i < $colspan; $i++) {
                        HTML2PDF::$_tables[$param['num']]['corr'][HTML2PDF::$_tables[$param['num']]['corr_y'] + $j][HTML2PDF::$_tables[$param['num']]['corr_x'] + $i] = ($i + $j > 0) ? '' : array($x, $y, $colspan, $rowspan);
                    }
                }
