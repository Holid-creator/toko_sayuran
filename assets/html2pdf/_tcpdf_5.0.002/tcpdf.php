...
if (!class_exists('TCPDF', false)) {
	...
	class TCPDF
	{
		...
		protected function UTF8StringToArray($str)
		{
			...
			for ($i = 0; $i < $length; ++$i) {
				$char = ord($str{
					$i}); // get one string character at time
		...
		{
			if (!$this->empty_string($url) and ($url{
				0} == '#')) {
				...
				if (preg_match($tagpattern, $element)) {
					...
					if ($element{
						...
						if (isset($dom[$key]['attribute']['style'])) {
							...
							// font style
							if (isset($dom[$key]['style']['font-weight']) and (strtolower($dom[$key]['style']['font-weight']{
								0}) == 'b')) {
								$dom[$key]['fontstyle'] .= 'B';
							}
							if (isset($dom[$key]['style']['font-style']) and (strtolower($dom[$key]['style']['font-style']{
								0}) == 'i')) {
								$dom[$key]['fontstyle'] .= 'I';
							}
							// font color
							...
							if (isset($dom[$key]['style']['text-align'])) {
								$dom[$key]['align'] = strtoupper($dom[$key]['style']['text-align']{
									0});
							}
						...
						if ($dom[$key]['value'] == 'font') {
							...
							if (isset($dom[$key]['attribute']['size'])) {
								if ($key > 0) {
									if ($dom[$key]['attribute']['size']{
										0} == '+') {
										$dom[$key]['fontsize'] = $dom[($dom[$key]['parent'])]['fontsize'] + intval(substr($dom[$key]['attribute']['size'], 1));
									} elseif ($dom[$key]['attribute']['size']{
										0} == '-') {
										$dom[$key]['fontsize'] = $dom[($dom[$key]['parent'])]['fontsize'] - intval(substr($dom[$key]['attribute']['size'], 1));
									} else {
						...
						if (($dom[$key]['value']{
							0} == 'h') and (intval($dom[$key]['value']{
							1}) > 0) and (intval($dom[$key]['value']{
							1}) < 7)) {
							$headsize = (4 - intval($dom[$key]['value']{
								1})) * 2;
						...
						if (isset($dom[$key]['attribute']['align']) and (!$this->empty_string($dom[$key]['attribute']['align'])) and ($dom[$key]['value'] !== 'img')) {
							$dom[$key]['align'] = strtoupper($dom[$key]['attribute']['align']{
								0});
						}
				...
				if ($dom[$key]['tag'] or ($key == 0)) {
					...
					if ((!$this->newline) and ($dom[$key]['value'] == 'img') and (isset($dom[$key]['attribute']['height'])) and ($dom[$key]['attribute']['height'] > 0)) {
						...
						if (($fontname != $curfontname) or ($fontstyle != $curfontstyle) or ($fontsize != $curfontsize)) {
							if (
								is_numeric($fontsize) and ($fontsize >= 0)
									...
									while (preg_match('/([0-9\.\+\-]*)[\s](Td|cm|m|l|c|re)[\s]/x', $pmid, $strpiece, PREG_OFFSET_CAPTURE, $offset) == 1) {
										...
										switch ($strpiece[2][0]) {
											...
											case 're': {
													...
													$pmid = preg_replace_callback(
														'/(' . $xmatches[1] . ')[\s](' . $xmatches[2] . ')[\s](' . $xmatches[3] . ')[\s](' . $strpiece[1][0] . ')[\s](re)([\s]*)/x',
											...
											case 'c': {
													...
													// justify block
													$pmid = preg_replace_callback(
														'/(' . $xmatches[1] . ')[\s](' . $xmatches[2] . ')[\s](' . $xmatches[3] . ')[\s](' . $xmatches[4] . ')[\s](' . $xmatches[5] . ')[\s](' . $strpiece[1][0] . ')[\s](c)([\s]*)/x',
														create_function('$matches', 'global $spacew;
			...
			switch ($name) {
				...
				case 'image': {
						...
						if (!$clipping) {
							...
							// fix image path
							if (!$this->empty_string($this->svgdir) and (($img{
								0} == '.') or (basename($img) == $img))) {
								// replace relative path with full server path
								$img = $this->svgdir . '/' . $img;
							}
							if (($img{
								0} == '/') and ($_SERVER['DOCUMENT_ROOT'] != '/')) {
